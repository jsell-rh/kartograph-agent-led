...
export function useMutationSubmission() {
  ...
  async function submit(jsonlContent: string, opCount: number, knowledgeGraphId?: string | null) {
    ...
    try {
      const result = await applyMutations(jsonlContent, { signal: controller.signal, knowledgeGraphId })
